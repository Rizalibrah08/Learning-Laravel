...
@section('content')
    ...
    @forelse($emp as $row)
        <tr>
            ...
            <td>
                @if($row->img)
                <img src="{{ asset('images/' . $row->img) }}" alt="foto {{ $row->nama }}" width="60" height="60" class="rounded-circle border">
                @else
            ...
            <td>
                ...
                <form action="{{ route('emp_delete',$row->id_emp) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                    @csrf
                    @method('DELETE')
